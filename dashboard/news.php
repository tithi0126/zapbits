...
if (isset($_REQUEST["flg"]) && $_REQUEST["flg"] == "del") {
    try {
        ...
        $stmt_del = $obj->con1->prepare(
            "DELETE FROM `news` WHERE `news_id`='" . $_REQUEST["n_id"] . "'"
        );
    ...
    if ($Resp) {
        if(file_exists("images/news/".$data["news_image"])){
            unlink("images/news/".$data["news_image"]);
          }
...
<div class='p-6 animate__animated' x-data='pagination'>
    <h1 class="dark:text-white-dar  pb-8 text-3xl font-bold">News</h1>
    <div class="panel mt-6 flex items-center  justify-between relative">
        ...
        <button type="button" class="p-2 btn btn-primary m-1 add-btn" onclick="javascript:insertData()">
            <i class="ri-add-line mr-1"></i> Add News</button>
        ...
        Alpine.data('pagination', () => ({
            ...
            init() {
                this.datatable = new simpleDatatables.DataTable('#myTable', {
                    data: {
                        headings: ['Sr.No.', 'Image', 'Title', 'Status','Short Description', 'Actions'],
                        data: [
                            <?php
                            $stmt = $obj->con1->prepare("SELECT * FROM `news` order by `news_id` desc");
                            $stmt->execute();
                            $Resp = $stmt->get_result();
                            $i = 1;
                            while ($row = mysqli_fetch_array($Resp)) {
                                ...
                                [
                                <?php echo $i; ?>,
                                '<img src="images/news/<?php echo addslashes($row["news_image"]); ?>" height="200" width="200" style="border-radius:10px">',
                                '<?php echo addslashes($row["title"]); ?>',
                                '<span class="badge whitespace-nowrap" :class="{\'badge-outline-success\': \'<?php echo $row["n_status"]; ?>\' === \'enable\', \'badge-outline-danger\': \'<?php echo $row["n_status"]; ?>\' === \'disable\'}"><?php echo $row["n_status"]; ?></span>',
                                `<p style="white-space: pre; width: 300px; overflow: hidden; text-overflow: ellipsis;"><?php echo addslashes($row["short_desc"]); ?></p>`,
                                getActions(<?php echo $row["news_id"]; ?>)
                                ],
                                <?php
                                $i++;
                            }
    ...
    async function showAlert(id) {
        new window.Swal({
            ...
            if (result.isConfirmed) {
                var loc = "news.php?flg=del&n_id=" + id;
                window.location = loc;
            }
        });
    }
