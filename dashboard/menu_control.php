...
if (isset($_REQUEST["flg"]) && $_REQUEST["flg"] == "del") {
    try {
        $stmt_del = $obj->con1->prepare(
            "delete from menu_control where id='" .
                $_REQUEST["m_controlid"] .
                "'"
        );
...
<div class='p-6 animate__animated' x-data='pagination'>
    <h1 class="dark:text-white-dar  pb-8 text-3xl font-bold">Menu Control</h1>
    <div class="panel mt-6 flex items-center  justify-between relative">
        <button type="button" class="p-2 btn btn-primary m-1 add-btn" onclick="javascript:insertdata()">
            <i class="ri-add-line mr-1"></i> Add Menu Control</button>
...
<script>
        ...
        Alpine.data('pagination', () => ({
            ...
            init() {
                this.datatable = new simpleDatatables.DataTable('#myTable', {
                    data: {
                        headings: ['Sr.No.', 'Name', 'Link', 'Status', 'Actions'],
                        data: [
                            <?php
                            $stmt = $obj->con1->prepare(
                                "SELECT * FROM `menu_control` order by id desc"
                            );
                            $stmt->execute();
    ...
    function insertdata(id){
        eraseCookie("edit_id");
        eraseCookie("view_id");
        window.location = "add_menu_control.php";
    }
    ...
    async function showAlert(id) {
        new window.Swal({
            ...
            if (result.isConfirmed) {
                var loc = "menu_control.php?flg=del&m_controlid=" + id;
                window.location = loc;
            }
        });
    }
